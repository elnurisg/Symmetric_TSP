...
dashes = ['-',  # solid line
    ...
    '-.',  # dash-dot line
    ':',  # dotted line
    '-',  # Add more dash styles as needed
    '--', 
    '-.',
    ':']

# Updated markers list with more markers
markers = ['+', 'x', 's', '^', 'o', 'd', '>', '<', 'v', 'p']
colors = ['r', 'b', 'y', 'g', 'm', 'c', 'k', 'orange', 'purple', 'brown']
...
def readTable(fp, delimiter):
    ...
    # Cycle through markers if ncols exceeds the length of markers list
    while ncols > len(markers):
        markers.extend(markers)  # Double the markers list to ensure enough markers
